...
// dum_callm: change bam input from 1 to multiple
...
// to compile
// gcc -Wall -g -O2 dum_callm.c -o dum_callm ../libhts.a -lz -lm -llzma -lcurl -lcrypto -lbz2
...
int parse_line(bam1_t *b, sam_hdr_t *hdr, khash_t(str) *h, int debug, khash_t(fasta) *fh, khash_t(dep) *dh, int no_snp_call){
  ...
  if (c->n_cigar) { // cigar
    ...
    for (i = 0; i < c->n_cigar; ++i) {
        ...
        if (bam_cigar_opchr(cigarp[i]) == 'H') noH = 0;
    }
  ...
  if (sa_info && noH){
    ...
    if (strcmp(chrom,sa_chrom)==0 && strcmp(strand, sa_strand) ==0) { // potential big deletion, could be insertion too, but update later
      ...
      if (all_pos1[0] > all_pos2[1] || all_pos1[3] >= all_pos2[2] || all_pos1[1] >= all_pos2[1]) goto fail;//{free(s.s); return 0;}
...
int main (int argc, char **argv)
{
  ...
  fprintf(output, "infile\tchrom\tref_start\tref_end\tref\talt\tsize\ttype\tmutCov\ttotalCov\tmutPercent\n");
  ...
  if (!no_snp_call) {
    if (fasta_file == NULL){
      ...
      fprintf(stderr,
        "Usage: snp_indel_check [options] -f your_sequence.fa <in.bam>|<in.sam>|<in.cram>\n"
        "Options:\n"
  ...
  if (fh != NULL){
    for (k = 0; k < kh_end(fh); ++k)
      if (kh_exist(fh, k)){
        ...
        char *vv = kh_val(fh, k);
        int n = strlen(vv) + 1; // first will be the array size
        int *tmp = malloc(n * sizeof(int)); // I have to put the n in pos 0 for later to know the size of array
        tmp[0] = n;
  ...
  for(; optind < argc; optind++){
    ...
    for (k = kh_begin(h); k != kh_end(h); ++k) { // traverse
        ...
        int vv = kh_val(h, k);
        if (vv >= min_cov) {
          fprintf(output, "%s\t%s\t%d", argv[optind], kk, vv);
          if (fh != NULL){
            ...
            int d1 = tmparray[ref_start+1];
            int d2 = tmparray[ref_end+1];
